...
COMMON_TCP_PORTS = [20, 21, 22, 23, 25, 53, 66, 67, 80, 135, 443, 445, 1900, 3389, 8000, 8080]
MAX_RTT = 2
RESEND_COUNT = 2
...
def send_syn(dst_ip, dst_port):
	...
	reply = scapy.sr1(syn_request, timeout=MAX_RTT, retry=RESEND_COUNT, verbose=0)
	return reply
...
def os_set_from_ip_layer(ip_layer, verbose=False):
	...
	if verbose:
		print "DF: {0}, TTL: {1}".format(df, ttl)
...
def os_set_from_tcp_layer(tcp_layer, verbose=False):
	...
	if verbose:
		print "WIN_SIZE: {0}, MSS: {1}".format(win_size, mss)
